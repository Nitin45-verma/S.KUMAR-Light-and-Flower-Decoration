...
class x {
  ...
  #p() {
    ...
    // Verify WebGL availability prior to instantiating WebGLRenderer
    const glTest = this.canvas.getContext('webgl2') || this.canvas.getContext('webgl') || this.canvas.getContext('experimental-webgl');
    if (!glTest) {
    ...
    this.renderer = new s(rendererOptions);
  ...
  #u(e) {
    this.#s = e[0]?.isIntersecting ?? false;
  ...
  resize() {
    ...
    this.size.width = e;
    this.size.height = t;
    this.size.ratio = e / Math.max(t, 1);
    this.#x();
    this.#b();
  ...
  #x() {
    ...
    this.camera.aspect = this.size.width / Math.max(this.size.height, 1);
...
function P(e, t) {
  ...
  i.x = A.x - t.left;
  i.y = A.y - t.top;
  s.x = (i.x / Math.max(t.width, 1)) * 2 - 1;
  s.y = (-i.y / Math.max(t.height, 1)) * 2 + 1;
}
function D(e) {
  const { x: t, y: i } = A;
...
function createBallpit(e, t = {}) {
  ...
  try {
    ...
    console.warn('Ballpit WebGL initialization failed gracefully:', err);
    return {
      three: null,
      spheres: null,
      setCount() {},
